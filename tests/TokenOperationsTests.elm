...
suite =
    describe "TokenOperations tests"
        ...
        , describe "validateEditState tests"
            [ test "validating the default state returns the nothing-selected options" <|
                \() ->
                    validateEditState 50 emptyEditState
                        |> Expect.equal (Just TokenOperationNotSelected)
            , test "validating a valid insert operation returns no error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = InsertTokensOption, insertTokenRange = makeEntry 10 20 }
                        |> Expect.equal Nothing
            , test "validating an insert operation with an invalid range returns an error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = InsertTokensOption, insertTokenRange = invalidEntry }
            ...
            , test "validating an insert operation with an empty range returns an error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = InsertTokensOption, insertTokenRange = makeEntry 20 10 }
            ...
            , test "validating an insert operation with a range off the end returns an error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = InsertTokensOption, insertTokenRange = makeEntry 45 55 }
                        |> Expect.equal (Just (RangeOffEndOfTokens InsertTokenRangeField))
            , test "validating a valid remove operation returns no error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = RemoveTokensOption, removeTokenRange = makeEntry 10 20 }
                        |> Expect.equal Nothing
            , test "validating a remove operation with an invalid range returns an error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = RemoveTokensOption, removeTokenRange = invalidEntry }
            ...
            , test "validating a remove operation with an empty range returns an error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = RemoveTokensOption, removeTokenRange = makeEntry 20 10 }
            ...
            , test "validating a remove operation with a range off the end returns an error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = RemoveTokensOption, removeTokenRange = makeEntry 45 55 }
                        |> Expect.equal (Just (RangeOffEndOfTokens RemoveTokenRangeField))
            , test "validating a valid swap operation returns no error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = SwapTokenRangeOption, swapTokenRange1 = makeEntry 10 20, swapTokenRange2 = makeEntry 30 40 }
                        |> Expect.equal Nothing
            , test "validating a swap operation with an invalid first range returns an error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = SwapTokenRangeOption, swapTokenRange1 = invalidEntry, swapTokenRange2 = makeEntry 30 40 }
            ...
            , test "validating a swap operation with an empty first range returns an error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = SwapTokenRangeOption, swapTokenRange1 = makeEntry 20 10, swapTokenRange2 = makeEntry 30 40 }
            ...
            , test "validating a swap operation with a first range off the end returns an error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = SwapTokenRangeOption, swapTokenRange1 = makeEntry 45 55, swapTokenRange2 = makeEntry 30 40 }
            ...
            , test "validating a swap operation with an invalid second range returns an error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = SwapTokenRangeOption, swapTokenRange1 = makeEntry 10 20, swapTokenRange2 = invalidEntry }
            ...
            , test "validating a swap operation with an empty second range returns an error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = SwapTokenRangeOption, swapTokenRange1 = makeEntry 10 20, swapTokenRange2 = makeEntry 40 30 }
            ...
            , test "validating a swap operation with a second range off the end returns an error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = SwapTokenRangeOption, swapTokenRange1 = makeEntry 10 20, swapTokenRange2 = makeEntry 45 55 }
            ...
            , test "validating a swap operation with overlapping ranges returns an error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = SwapTokenRangeOption, swapTokenRange1 = makeEntry 10 20, swapTokenRange2 = makeEntry 15 25 }
            ...
            , test "validating a swap operation with ranges overlapping by a single value returns an error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = SwapTokenRangeOption, swapTokenRange1 = makeEntry 10 20, swapTokenRange2 = makeEntry 20 30 }
                        |> Expect.equal (Just SwapTokenRangesOverlap)
            , test "validating a swap operation with both ranges invalid returns an error with the first range" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = SwapTokenRangeOption, swapTokenRange1 = invalidEntry, swapTokenRange2 = invalidEntry }
                        |> Expect.equal (Just (InvalidRange SwapTokenRangeField1))
            , test "validating a swap operation with the first range off the end and overlapping the second returns the off-the-end error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = SwapTokenRangeOption, swapTokenRange1 = makeEntry 35 55, swapTokenRange2 = makeEntry 30 40 }
                        |> Expect.equal (Just (RangeOffEndOfTokens SwapTokenRangeField1))
            , test "validating a swap operation with the second range off the end and overlapping the first returns the off-the-end error" <|
                \() ->
                    validateEditState 50 { emptyEditState | operation = SwapTokenRangeOption, swapTokenRange1 = makeEntry 30 40, swapTokenRange2 = makeEntry 35 55 }
                        |> Expect.equal (Just (RangeOffEndOfTokens SwapTokenRangeField2))
            ]
        ]
